package com.xindu.talkfx_new.widget;
...
//        }
    ...
    private void drawTitle(Canvas canvas) {
        ...
        float textRadius = radius + fontHeight * 2;
        double pi = Math.PI;
        for (int i = 0; i < count; i++) {
            float x = (float) (centerX + textRadius * Math.cos(angle * i));
            float y = (float) (centerY + textRadius * Math.sin(angle * i));
            if (i == count - 1) {
                x = (float) (centerX + textRadius * Math.cos(angle * 4) + 120);
                y = (float) (centerY + textRadius * Math.sin(angle * 3));
            } else if (i == 0) {
                x = (float) (centerX + textRadius * Math.cos(angle * i) + 40);
            }
            //当前绘制标题所在顶点角度
